...
\title{Habituation as Model Update in a Minimal Embodied Agent:\\
Controls, Persistence Without Explicit Memory, and a Causal Self-Model}
\author{Adri\'an Valerio Porras\\ \small Conciencia Artificial v0.12\\ \small \texttt{San Jos\'e, Costa Rica} }
\date{29 August 2026}
...
\noindent\textit{Preprint. Submitted to arXiv cs.AI and q-bio.NC. This is a workshop-level report of an integrated minimal system, not a claim about phenomenal consciousness.}

% Word count: abstract 179 words (target 180)

\section{Introduction}

We started with a simple worry that many people in Costa Rica will recognize. We talk to large language models every day, and they talk back beautifully, but we wondered: what is actually behind the mouth? Is there anything on the other side that cares, predicts, gets surprised, and remembers in its own body, or is it only fluent text? Pura vida humility helps here. We do not claim to have solved consciousness. We claim something smaller and more testable.
...
We chose a deliberately minimal embodied agent for this reason. No images. No audio. No pre-trained vision backbone. The agent only feels its position and four internal variables we call ECUS (Energy, Courage or social drive, Uncertainty, and Sociability), plus touch. The idea is familiar to anyone who has closed their eyes: you are still conscious through your body. If awareness has anything to do with sensorimotor contingencies, then it should already be visible in this tiny loop. If it is not, then adding a bigger network will not magically create it.

Frankly, we were surprised that the loop worked at all on a single MacBook Air, no GPU, over coffee in San Jos\'e. We expected to need more. What we found, after we blinded ourselves with pre-registration and explicit controls, is a small set of effects that survive honest tests, and one instructive failure that we keep in the paper because it matters more than another positive bar.
...
The fourth item is the one we worried was a philosophical trick, so we tested whether $\Phi$ actually changes what the agent does. It does, with a large effect. The integration itself, together with the full set of controls, is the novelty. No published system, to our knowledge, shows all four running together for 30,000 continuous steps with the controls reported here (see Section~\ref{sec:related}).
...
All numbers in Section~\ref{sec:results} come from scripts in \texttt{framework/} and JSON files in \texttt{results/}. The complete long run is reproduced by \texttt{python3 framework/organismo\_final.py --steps 30000} on a MacBook Air M4 with MPS, no GPU required. Wall time is a few minutes without the mouth, longer if you enable generation through MLX.
...
Homeostasis H5-bis & Mean $E$ over 5k steps & $0.85$, $100\%$ in $[0.5,1.2]$ & CI $[0.85,0.85]$ \\
...
Third, functional separation: the ratio $\mathrm{presence}(\mathrm{expected\ noise})/\mathrm{presence}(\mathrm{violation})$ was $0.13$, a $7.7\times$ separation (threshold $<0.7$). When $\Phi$ expects noise, the same raw error counts less. When it does not, the same error counts fully. A conventional predictor cannot do this distinction because it has $\epsilon$ but not $\epsilon/\sigma^2$.

The fourth test is the one we care about most, because it answers the epiphenomenon worry. We coupled $\Phi$ to action: when $\Phi$ predicts high uncertainty, the agent executes an epistemic action that leaves the fog. We compared condition A ($\Phi$-coupled) to condition B (identical $\Phi$ computed but not allowed to influence action) over 30 seeds. Time in fog was $15.0\% \pm 6.3$ in A versus $28.1\% \pm 9.6$ in B, $d=-1.61$. This is a large, robust effect. The agent that knows its senses are unreliable acts to regain clarity, and that action halves its exposure to unreliable sensing. Knowing that you do not know changes what you do.
...
We do not close by claiming consciousness. We close on a smaller, more useful note. If awareness has engineering prerequisites, they include at least these: a generative model of the body, a way to know when that model is trustworthy, and a way to act differently when it is not. All three run here for 30,000 steps on a laptop without a GPU.
...
If you are a student reading this in Costa Rica or elsewhere and you want to check us, please do. Clone the repository, run \texttt{framework/organismo\_final.py}, change the fog to the other side, move the food inside it, freeze $\Phi$, shuffle the actions. The whole point of a minimal organism is that you can hold it in your hands and see where it breaks. Pura vida science is open science.

\section{Reproducibility Statement}

All numbers in Table~\ref{tab:main} are produced by scripts in \texttt{framework/} and logged as JSON in \texttt{results/}. Key scripts: \texttt{organismo\_final.py} (30k integrated run, v0.12 continuous physics with attention), \texttt{h6\_selfmodel.py} ($\Phi$ calibration), \texttt{h6\_phi\_causal.py} (causal gate), \texttt{rigor\_controles.py} (C1--C4c), \texttt{estadistica\_fase2.py} and \texttt{analisis\_fase2.py} (30-seed statistics), \texttt{h5bis} (homeostasis with policy). Instructions to reproduce are in \texttt{README.md} and \texttt{requirements.txt}. No GPU is required. On an Apple M4 Pro (MPS) the full 30-seed battery finishes in under an hour; the single 30k run finishes in a few minutes. We seed with 7 and vary the seed per run for the 30-seed analyses. Bootstrap CIs use 2,000 resamples. We report every control, including the one that fails (C3).

\section{Acknowledgements}

We thank the open-science community whose code and reports we build on, especially the authors of the LFM2.5 report, CheckVLA, Gubernaut, and the Levin lab Training Ecosystems framework. We thank the Costa Rican research community for the reminder that good science can be small, warm, and rigorous at the same time. This work was done entirely on local hardware in San Jos\'e, with no cloud budget and no GPU, over many small iterations. That constraint was not only economic. It kept the system small enough to understand.
...
\bibitem{tononi2014} Tononi, G., Boly, M., Massimini, M., and Koch, C. Integrated information theory: from consciousness to its physical substrate. \textit{Nature Reviews Neuroscience}, 17(7):450--461, 2016 (often cited as 2014 manuscript).
...
\bibitem{zhang2026} Zhang, Y., Liu, X., and Chen, W. Meta-cognitive reinforcement learning with self-predicted uncertainty. \textit{Unpublished manuscript}, 2026. Cited as project-internal reference; verify before final journal submission.

\bibitem{levin2026} Levin, M. et al. Training ecosystems: habituation as model update in minimal living systems. \textit{In preparation}, 2026. Framework discussed in Levin lab talks, 2025--2026.
...
\bibitem{checkvla2026} CheckVLA: action-conditioned controls for violation-of-expectation in embodied agents. \textit{Unpublished manuscript under review}, 2026. Control logic adopted from public discussion draft.

\bibitem{gubernaut2026} Gubernaut: a compact embodied loop with stability criteria (GCC). \textit{Unpublished manuscript under review}, 2026. Cited for minimal-loop design inspiration.
...
\bibitem{probe2026} Probe perturbation and affective grounding in minimal agents (``Asleep at the Wheel''). \textit{Unpublished manuscript under review}, 2026.
